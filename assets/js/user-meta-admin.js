(function($) {
    ...
    userMeta.admin = {
        ...
        events: function() {
            $(document).on('click', '.card .card-heading', this.togglePanel);
        },
    ...
    userMeta.formEditor = {
        ...
        load: function() {
            $('#um_fields_container').sortable({ handle: ".card-heading" });
        ...
        fieldsEvents: function() {
            ...
            this.editor.on('click', '#um_fields_selectors .card-heading', this.toggleSelectorPanel);
        ...
        events: function() {
            ...
            this.editor.on('click', '.card .card-heading .um_trash', this.removePanel);

            this.editor.on('change', '.um_parent', this.toggleConditionalConfig);
        },

        expandFirstField: function() {
            $('#um_fields_container .card-collapse').removeClass('in');
            $('#um_fields_container .card-collapse').first().addClass('in');

            $('#um_fields_selectors .card-collapse').first().addClass('in');
        },
        ...
        sanitizeSelectors: function() {
            var first = $('#um_fields_selectors .card-collapse').first();
            first.addClass('in');
        ...
        toggleSelectorPanel: function() {
            self = $(this).closest(".card").find(".collapse");
            $(this).closest("#um_fields_selectors").find(".collapse").not(self).slideUp();
        },

        removePanel: function() {
            if (confirm('Confirm to remove?')) {
                if (userMeta.formEditor.name == 'form_editor') {
                    var fieldID = $(this).closest(".card").find('.um_field_id').text();
        ...
        changeField: function() {
            var field = $(this).closest('.card');
            var id = $(field).find('.um_field_id').text();

            var fieldObj = $(this).closest('.card-body').find('input, textarea, select').serializeArray();
            var arg = {};
        ...
        changeTitle: function() {
            ...
            $(this).closest(".card").find("h3 .um_field_label").text(title);
        },

        updateMetaKey: function() {
            self = $(this).closest('.card');
        ...
        loadConditionalConfig: function() {
            this.editor.find('.card-body .um_parent').each(function() {
        ...
        toggleConditionalConfig: function(event, input) {
            ...
            if (tagName == 'select') {
                ...
                $(allChild).each(function() { //console.log(card.find( 'input[name='+ this +']' ).closest('p'));
                    userMeta.formEditor.toggleHideFieldBuilderFields(card, this);
                });
                ...
                if (input.find(':selected').data('child')) {
                    ...
                    $(targetChild).each(function() {
                        userMeta.formEditor.toggleShowFieldBuilderFields(card, this);
                    });
                }

            } else if (tagName == 'input' && input.attr('type') == 'checkbox') {
                targetChild = input.data('child').split(',');
                $(targetChild).each(function() { //card.find( 'input[name='+ this +']' ).hide();
                    if (input.is(":checked")) {
                        userMeta.formEditor.toggleShowFieldBuilderFields(card, this);
                    } else {
                        userMeta.formEditor.toggleHideFieldBuilderFields(card, this);
        ...
        toggleHideFieldBuilderFields: function(card, inputName) {
            card.find('input[name=' + inputName + ']').closest('.um_fb_field').slideUp();
            card.find('select[name=' + inputName + ']').closest('.um_fb_field').slideUp();
        },

        toggleShowFieldBuilderFields: function(card, inputName) {
            card.find('input[name=' + inputName + ']').closest('.um_fb_field').slideDown();
            card.find('select[name=' + inputName + ']').closest('.um_fb_field').slideDown();
        },

        toggleConditionalPanel: function() {
            var card = $(this).closest('.card-body').find('.um_conditional_details');
            if ($(this).is(":checked")) {
                card.slideDown();
            } else {
                card.slideUp();
            ...
            userMeta.formEditor.conditionsCountsEvent(card);
        },

        conditionalPlus: function() {
            var card = $(this).closest('.um_conditional_details');
        ...
        conditionsCountsEvent: function(card) {
            var rows = card.find('.um_conditions .form-group').length;
            if (rows > 1) {
                card.find('.um_conditional_relation_div').slideDown();
                card.find('.um_conditional_minus').show();
            } else {
                card.find('.um_conditional_relation_div').slideUp();
                card.find('.um_conditional_minus').hide();
            }
        },

        addOptionToConditions: function(id) {
            var field = $('#um_admin_field_' + id + ' .card-title');

            var optionLabel = $('#um_admin_field_' + id + ' .card-title .um_field_card_title').text();
    ...
    userMeta.optionsSelection = {
            ...
            toggleAdvancedMode: function() {
                ...
                if (card.find('.um_selector_options').length > 0) {
                    card = card.find('.um_selector_options');

                    if ($(this).is(":checked")) {
                        card.find('.um_advanced').show('slow').css('display', 'inline-flex');
                    } else {
                        card.find('.um_advanced').hide('slow');
                    }
                }
            },
        ...
        userMeta.addons = {
            ...
            toggleAddon: function(event, state) {
                ...
                userMeta.addons.ajaxSave(attr, card, function(data) {
                    if (state) {
                        card.find(".um_ribbon").removeClass('invisible').addClass('visible');
                        card.find(".um_addon_icon").removeClass('inactive').addClass('active');
                        card.find(".um_options").removeClass('invisible').addClass('visible');
                        window.location.reload();
                    } else {
                        card.find(".um_ribbon").removeClass('visible').addClass('invisible');
                        card.find(".um_addon_icon").removeClass('active').addClass('inactive');
                        card.find(".um_options").removeClass('visible').addClass('invisible');
                    }
                });
            },
...
function umRemoveFieldToExport(element, formID) {
    if (confirm("This form will removed permanantly. Confirm to Remove?")) {
        ...
        jQuery(element).parents(".card").hide('slow').empty();
    }
}
...
function umShortcodeIcon(element) {
    jQuery(element).find('button').show();
    jQuery(element).mouseleave(function(){ jQuery(element).find(':button').hide(); });
}