...
class Login
{
    ...
    private function defaultLoginForm()
    {
        ...
        $html .= $userMeta->renderPro('loginForm', array(
            'config' => $this->config,
            'loginTitle' => ! empty($title[$this->settings['login_by']]) ? $title[$this->settings['login_by']] : null,
    ...
    private function loginResponse($user = null)
    {
        ...
        return $userMeta->convertUserContent($user, (! empty($this->settings['loggedin_profile'][$role]) ? $this->settings['loggedin_profile'][$role] : null));
    }
    ...
    public function changeLoginErrorMessage($user, $username, $password)
    {
        ...
        if (in_array((! empty($this->settings['login_by']) ? $this->settings['login_by'] : null), [
            'user_email',
            'user_login_or_email'
        ])) {
            $title = $userMeta->loginByArray();
            ...
            if ($user->get_error_code() == 'invalid_username')
                return new \WP_Error('invalid_username', sprintf(__('<strong>ERROR</strong>: Invalid %s.', $userMeta->name), (! empty($title[$login['login_by']]) ? $title[$login['login_by']] : null)));
    ...
    function findUserForLogin()
    {
        ...
        if ($user === false) {
            $title = $userMeta->loginByArray();
            return new \WP_Error('invalid_login', $userMeta->getMsg('invalid_login', (! empty($title[$loginBy]) ? $title[$loginBy] : null)));
        }
