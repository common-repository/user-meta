...
class LibRawFunctionsModel
{
    ...
    function createInput($name = '', $type = 'text', $attr = array(), $options = array())
    {
        ...
        if (in_array($type, $excludeType))
            $excludeAttr[] = 'value';
        $include = null;
        if (is_array((! empty($attr) ? $attr : null))) {
        ...
        if ($type == 'select') {
            ...
            if (is_array((! empty($options) ? $options : null))) {
                $i = 0;
                foreach ($options as $key => $val) {
                    if (! $by_key)
                        $key = $val;
            ...
            $form_id = ! empty($attr['form_id']) ? $attr['form_id'] : null;
            if ($form_id) {
    ...
    function arrayRemoveEmptyValue($array, $keepEmptyArray = false)
    {
        ...
        foreach ($array as $key => $val) {
            ...
            if (! empty($result[$key]) && is_string((! empty($result[$key]) ? $result[$key] : null)))
